...
def generate_sobol_tiles(dupe_num):
    """
    This augments the pnpoly_tiles.csv file with an index into the 
    sobol sequence file.

    We write the augmented dataset out to 'bssid_sobol_idx.csv'.

    The index into the sobol sequence gives us a starting point from
    which to generate new randomized IDs.
    """

    FILE_LEN = file_len('sobol_seq.csv')
    with open('bssid_sobol_idx.csv', 'w') as fout:
        writer = csv.writer(fout)
        for (bssid, tx, ty, ignore_z) in csv.reader(open('pnpoly_tiles.csv')):
            idx = randint(0, FILE_LEN)
            writer.writerow((bssid, tx, ty, ignore_z, idx))


def obfuscate_tile_data(dupe_num):
    tile_gen = generate_sobol_tiles(dupe_num)
...
if __name__ == '__main__':
    ...
    v = load_path('../shapefiles/toronto/toronto.geojson')
